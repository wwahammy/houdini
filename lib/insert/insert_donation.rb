...
module InsertDonation
  ...
  def self.offsite(data)
    ...
    off_t = trx.subtransactions.build(subtransactable: 
      OfflineTransaction.new(amount: data['amount'], created: data['date']), 
      subtransaction_entities:[
        SubtransactionEntity.new(
          entitiable: OfflineTransactionCharge.new(amount:data['amount'], created: data['date']),
          payments: [Payment.find(result['payment']['id'])]
      )
      ]);
    trx.save!
    don.save!
    off_t.save!
    off_t.subtransaction_entities.each{|se| se.publish_created}
    off_t.payments.each{|py| py.publish_created}
